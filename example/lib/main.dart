...
class _LongScreenshotDemoState extends State<LongScreenshotDemo> {
  ...
  Future<void> _captureAndSaveAsPdf() async {
    ...
    try {
      ...
      if (mounted) {
        final String platformMessage = Platform.isIOS
            ? 'PDF saved to Documents and shared'
            : 'PDF saved: $pdfPath';
  ...
  Future<void> _captureAndSaveToDownloads() async {
    ...
    try {
      ...
      final String filePath =
          await FlutterLongScreenshot.saveToDownloadsAndOpen(
            imageData: imageData,
            fileName:
                'long_screenshot_${DateTime.now().millisecondsSinceEpoch}',
          );
      ...
      if (mounted) {
        final String platformMessage = Platform.isIOS
            ? 'Screenshot saved to Documents and shared'
            : 'Screenshot saved to Downloads: $filePath';
